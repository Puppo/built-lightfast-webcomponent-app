...
export class AppShell extends TemplateRender {
    ...
    async getPeople() {
        this.people = await fetch('https://randomuser.me/api?results=50').then(res => res.json()).then(res => res.results);
    ...
    get template() {
        ...
        <profile-card-animator>
            ${this.people ? this.people.map(person => `<profile-card profile-data="${super.encodeAttributeObject(person)}"></profile-card>`).join('') : ''}
        </profile-card-animator>
        `
    }
}